...
class CharacterLoader:
        ...
        for char_file in self.characters_dir.glob("*.json"):
            try:
                ...
                characters.append({
                    ...
                    "description": char_data.get("short_description", "Описание недоступно"),
                    "file": str(char_file)
                })
    ...
    def load_character(self, character_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if character_id in self.relationship_history:
                character_data['current_relationship'] = self.relationship_history[character_id]
            else:
                # Создаем новую историю отношений
                character_data['current_relationship'] = self._create_new_relationship(character_data)
                self.relationship_history[character_id] = character_data['current_relationship']
                self._save_relationship_history()
            
            self.current_character = character_data
            self._save_current_character(character_id)
            
            self.logger.info(f"Персонаж загружен: {character_data.get('name', character_id)}")
            return character_data
            
        except Exception as e:
            self.logger.error(f"Ошибка загрузки персонажа {character_id}: {e}")
            return None
    ...
    def switch_character(self, new_character_id: str) -> bool:
        ...
        if self.current_character:
            ...
            if current_id in self.relationship_history:
                self.relationship_history[current_id] = self.current_character.get('current_relationship', {})
    ...
    def get_character_context_for_ai(self) -> str:
        ...
        if char.get('speech'):
            ...
            if speech.get('catchphrases'):
                context_parts.append(f"Фразочки: {', '.join(speech['catchphrases'][:3])}")
    ...
    def _create_new_relationship(self, character_data: Dict[str, Any]) -> Dict[str, Any]:
